...
void uchar_to_ulong(const uchar *input, uint input_len, ulong *output)
{
  uint num_ulongs = (input_len + 7) / 8; // Calcula o número de ulong necessário
  for (uint i = 0; i < num_ulongs; i++)
  {
    ...
    for (uint j = 0; j < 8; j++)
    {
      ...
      if (index < input_len)
      {
        output[i] |= ((ulong)input[index]) << (56 - j * 8);
      }
    }
  }
}
